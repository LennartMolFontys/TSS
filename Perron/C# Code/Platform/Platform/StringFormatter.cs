...
namespace Platform
{
    static class StringFormatter
    {
        ...
        public static List<List<int>> GetSeatsTaken(string someString)
        {
            try
            {
                string[] unitStrings = someString.Split(seatOccupationHeaders, StringSplitOptions.RemoveEmptyEntries);
                List<List<int>> unitOcupation = new List<List<int>>();

                foreach (string unitString in unitStrings)
                {
                    List<int> intValues = new List<int>();
                    string[] values = unitString.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                    foreach(string stringValue in values)
                    {
                        intValues.Add(Convert.ToInt32(stringValue));
                    }
                    unitOcupation.Add(intValues);
                }
                return unitOcupation;
            }
            catch (FormatException)
            {
